...
ORmodelrun<-function(model_data,DependentVar,model_vec,model_names){
  ...
  model_data %>%
    dplyr::select(DependentVar,Chronotype) %>% table(useNA = "always") %>%
    as.data.frame() -> tab_1
...
model_data %>% mutate(Chronotype = ifelse(Chronotype%in% c("More a morning than an evening person",
  ...
  mutate(Chronotype=factor(Chronotype,levels=c("Intermediate chronotype",
                                               ...
                                               "Do not know",
                                               "Prefer not to answer")))-> model_data
# running -----------------------------------------------------------------

model_vec<-c("Chronotype + Sex + Year_of_birth",
             #"JiNS + Sex + Year_of_birth + TDI + SleepDur + Packyears + Alcintake + LengthofWW",
             "Chronotype + Sex + Year_of_birth + Smoking + Alcohol + Ethnicity + TDI + DaysWalked + DaysModerate + DaysVigorous + BMI + Packyears + Alcintake + LengthofWW + Job_AsthmaRisk + Job_MedRequired",
             "Chronotype + Sex + Year_of_birth + Smoking + Alcohol + Ethnicity + TDI + SleepDur + DaysWalked + DaysModerate + DaysVigorous + BMI + Packyears + Alcintake  + LengthofWW + Job_AsthmaRisk + Job_MedRequired")
...
save(table3_WWJiNS,table4_FEVJiNS,tab6asthmadef,tab7asthmadefms,
     file="data/chronotablesdata_inter.RData")
